...
/** Number of shows surfaced in the "Recommended for you" carousel. */
 ...
 * @property {Array<Object>} recommendedShows - Randomised subset of all shows for the homepage carousel.
...
export function useShowListQuery() {
  ...
  // Recomputed only when the fetched show list identity changes (i.e. once,
  // after the initial load) — not on every filter/sort/page change — so the
  // carousel's order doesn't jump around as the user searches or paginates.
  const recommendedShows = useMemo(() => {
    return shuffle(allShows)
      .slice(0, RECOMMENDED_COUNT)
      .map((show) => ({ ...show, genreTitles: getGenreTitles(show.genres) }));
  }, [allShows]);
